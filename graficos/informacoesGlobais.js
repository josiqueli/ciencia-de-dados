...
async function vizualizarInformacoesGlobais() {
    ...
    const pessoasEnsinoS = (dados.total_pessoas_com_educacao_superior / 1.5e9)
    const minutos = Math.round((dados.tempo_medio - horas) * 100)
    const porcentagemConectada = ((pessoasEnsinoS / pessoasNoMundo ) * 100).toFixed(2)

    const paragrafo = document.createElement('p')
    paragrafo.classList.add('graficos-container__texto')
    paragrafo.innerHTML = `Você sabia que o mundo tem <span>${dados.total_pessoas_mundo}</span> de pessoas e que aproximadamente <span>${dados.total_pessoas_com_acesso_a_educacao} bilhões</span> tem acesso à alguma rede de educação e passam em média <span>${horas} horas</span> estudando.<br>Desse total, apenas <span>${porcentagemConectada}%</span> de pessoas tem ensino superior.`
